...
**Status:** ✅ **DELIVERED AND ARCHIVED 2026-08-10 — 8 of 8 criteria, and the exception that kept the SCOPE open is discharged.** One commitment travels out rather than closing here: [the live rotation cell](../../backlog/2026-08-10-rotation-has-no-trigger-and-its-live-cell-was-never-run.md).

> **The two sentences below are the status this file carried until close-out. They are kept because the first one is now FALSE, and how it went false is the record.**
>
> _"**STEPS 4 AND 5 SHIPPED (session 11). Grapevine no longer participates in CONVENE or JOIN** — it is still consulted at **teardown** (`team-support.ts:469`, `grapevine who`, fired by `status` and `down`), which is why the sentence is scoped rather than absolute."_ 🔴 **FALSIFIED 2026-08-08 at `4d091dc`** — the vine leg was dropped from the presence path. The scoping clause was correct when written and is now describing a call site that does not exist. **Verified at close-out, not inferred: the only surviving executable `grapevine` in `plugin/scripts/` is the type-union member `CoordTool = "grapevine" | "bounty"` (`coord.ts:22`), and `resolveCoordCli`'s sole remaining wire is narrowed to `(tool: "bounty")` at `team-join.ts:668`.**
>
> _"**✅ SESSION 12 CLOSED THE RELEASE BAR — 6 of 6** … only criterion 7 (the re-triage) remains."_ 🔴 **FALSIFIED as forward-looking** — criterion 7 was met at session 13 (2026-08-08), which this file's own sweep box 470 lines below already recorded. **The header and the sweep box disagreed for two days, and the header is the part a stranger reads first.** _That is the drift the sweep beat exists to catch, occurring in the document the beat had already been pointed at — the second time this file has recorded exactly that, and this time between its own head and its own foot rather than between two documents._

**`convene` opens nothing, `join` composes no vine tail, the whole C4 prose surface is migrated, and the presence path is comms-only. Gate item 3 — the pane-kill — is DISCHARGED**: INFERRED since session 9, reproduced and instrumented at `c9a33e7`, then run live at session 12 (7 cells, real tmux).
...
**✅ NOTHING REMAINS IN THIS PROJECT.** Criterion 7 closed at session 13 with a named exception (a live `grapevine who` on the presence path); **that exception was itself discharged at `4d091dc`, which is what let the SCOPE close and not just the criterion.** The [NEXT PHASE](#-next-phase--what-session-13-picks-up) section at the foot is a **record of session 13's runway, not an instruction** — its own sweep box says so.

> **⏭ THE ONE THING THAT TRAVELS:** the live rotation cell — filed as
> [`2026-08-10-rotation-has-no-trigger-and-its-live-cell-was-never-run`](../../backlog/2026-08-10-rotation-has-no-trigger-and-its-live-cell-was-never-run.md).
> **Measured at close-out and NOT known when this plan was written: `rotateSession` has zero production callers.** So "rotation landed inert" understates it — there is no verb, hook, or command that can fire it, which is why the decisive cell stayed owed. **A library function with full test coverage and no caller is green by every measure this repo runs.**
...
drop it on the strength of rotation having landed.**

2. **Criterion 2's absence-of-OPENING half has NO runtime artifact, and this was measured.** On an
   ...
   **The domain matters:** on an **archived** channel `open` auto-unarchives and IS observable. Our
   ...
   sentence is false and was corrected once already.

3. **The artifact is the WHOLE SPAWN SET, never the absence of `grapevine`.** sentinel certified this
   and mutation-proved it: **`not.toContain("grapevine")` passes VACUOUSLY in CI** — a green that can
   never fail, inside the assertion built to catch false greens.

4. **The pane-kill guard is verified at the COMMAND boundary and not in a live session** — L1
   ...
   open with its wording intact.

5. **Cite CARD IDs, never S-numbers.** S-numbers collided across two authors within one hour
...
| —                           | `grapevine who` at `commands/team-support.ts:469`  | ✅ **DISCHARGED 2026-08-08 at `4d091dc` — and it WAS a decision, not a side effect, which is what this row asked for.** The case analysis now lives at `team-support.ts:621` as a two-leg enumeration: the vine could change the verdict in **exactly one cell** (`vine unknown` × `comms none`), `present` was unreachable for the vine once nothing joined the channel, **and that one cell was a bug rather than a safety margin** — `resolveCoordCli` throws when spellbook is absent, which is the ordinary state of a consuming project, so `down` blocked every clean teardown and demanded `--force`. `humans` went with the leg, ruled the same day: _anthill does not give agents visibility into the human — the arrow points the other way._ **The original text of this row follows, because its warning was correct and is the reason the removal was argued rather than assumed:** 🔴 **THE LAST VINE CALL SITE, AND IT IS A LOAD-BEARING MASK — read this before removing it.** Measured session 12, one variable (`HOME`), live tmux both times: **vine resolves → `down` authorises and kills; vine UNRESOLVABLE → `down` BLOCKS, always.** `combinePresence` needs BOTH legs `none` before anything authorises, so **an unresolvable vine leg makes `down` always-block on any machine without spellbook.** Removing it means `commsPresence` becomes the **sole** input and every fail-safe property rests on it alone. **Not an argument to keep it** — telling a comms-only team _"grapevine CLI unresolved"_ is its own defect — **but it must be a decision, not a side effect.** _A mask is not a dependency; you cannot find these by tracing what depends on what._ Removal is a presence-semantics change with a ratify gate, and belongs to the [coordination-layer investigation](../../investigations/2026-07-31-team-native-coordination-layer.md) |
...
## Open questions — settled at close-out, 2026-08-10

**Each carries its verdict. `UNCHECKED` is a real verdict and is written where it applies.**

1. **C1's `humans` capability** — deleted, deferred, or blocking? → ✅ **SETTLED: DELETED.** Ruled 2026-08-08 and executed with the vine leg at `4d091dc`. The reasoning is not "it was cheap": _anthill does not give agents visibility into the human — the arrow points the other way_, so the capability was **nominal and pointed the wrong direction.** It was flagged to Cole as a user-visible deletion when this plan predicted it would be one.
2. **The migrate guard's circularity** → ⏭ **STILL OPEN, and it outlives this project.** It is a presence-semantics question about `migrate`, not about the wire swap. Not carried as a new item because nothing here made it more urgent; it belongs with the [coordination-layer investigation](../../investigations/2026-07-31-team-native-coordination-layer.md).
3. **`--fresh` vs a session mint** → ✅ **MOOT BY SUBJECT DELETION.** `convene --fresh` was deleted with step 4, so the question has no subject. This is the `MOOT` class steward named at S13-E — _"we fixed it"_ and _"the thing it was about is gone"_ send a fresh agent to different places. Card `t-e25cd535` is the same fact and closes on inspection.
4. **`.claude/settings.local.json` pins a grapevine CLI path at 1.14.0** → ✅ **MOOT, and re-measured rather than assumed.** The file still carries the pin (line 16, a `grapevine … send` permission), but **anthill never invokes grapevine on any path**, so a stale permission entry for a tool we do not call costs nothing. Untracked and out of every sweep's reach, which is why it is recorded here rather than fixed.
5. **Session 9 has no failure branch** → ⏭ **STILL UNDECIDED, and the swap held**, so it was never forced. Deferred to the shape check when it was written and never taken up. Recorded as genuinely unanswered rather than closed by the happy path.
