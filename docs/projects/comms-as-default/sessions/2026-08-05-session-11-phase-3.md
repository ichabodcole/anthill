...
**2. The pane-kill is a THREE-term conjunction, and the middle term is our own ritual.** `positions blinded ∧ in-window tombstones ∧ vine empty`. **Rotation alone fails CLOSED** — `plan.md:72` is wrong in the safe direction. Since D2 made `stand-down` advisory, _a seat that files its tombstone and keeps working_ **is** the middle term. **Rotation is not the hazard; rotation after finalize has begun is the hazard.**
