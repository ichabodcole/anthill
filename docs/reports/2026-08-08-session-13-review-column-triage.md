...
| ------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `t-ce6f0c2e` | **rotation is UNWIRED, not merely un-run.** `rotateSession` exists at `comms.ts:289`; no caller anywhere in `commands/`; the live tree is still the LEGACY layout (`anthill-dev.positions/`, no `CURRENT` pointer). It cannot be executed without new code.             |
