...
**Added:** 2026-08-01 · **Status:** ✅ **SHIPPED** 2026-08-01 (`f5668cb`) — both defects fixed, each
with a guard verified against the pre-fix behaviour ·
...
- [x] A bad flag produces a usage error, not `ok:true` — call sites audited first: **no command reads
      an undeclared flag**, so nothing depended on the swallow. The `catch` was the real landmine and
      was fixed in the same pass (it fell back to treating every argument as a positional).
- [x] A gate-failure bounce emits the envelope and **no stack frames**, guarded on the throwing path.
      ⚠ **The fix introduced a regression, caught in review:** `process.exit()` does not unwind
      `finally`, so the emit+exit dropped the lock release — stranding the serialize lock and blocking
      every peer for 90s. Reproduced, fixed, and guarded by two further tests.
- [x] Neither fix landed inside the comms spike branch — both went to `develop` via
      `fix/commit-attribution-and-visibility`.
