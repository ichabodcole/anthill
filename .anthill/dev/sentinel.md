...
- **A PROTECTION CAN BE SCOPED TO A WORLD YOU ARE IN THE MIDDLE OF CHANGING, and the clause will not say so.**
  Contract 6(g) property 1: a lead who is in the roster and not in `spawned` *"blocks teardown with his own live follower until he stands down himself."* **Measured both sides: TRUE pre-rotation (`present`, `seats:["maestro"]`, blocking alone) and FALSE post-rotation (he contributes nothing; `none`).** His liveness is observable only THROUGH a position record, and rotation deletes it.
  _**The lead's veto is not surrendered by a decision — it is deleted by a maintenance operation on someone else's card.** Nobody predicted it, including the clause's own owner and the seat re-ratifying it._
  _Reflex: when a clause states a protection, ask **which world it was measured in**, and whether this session is changing that world. This is the domain question pointed at a GUARANTEE rather than at a definition._
