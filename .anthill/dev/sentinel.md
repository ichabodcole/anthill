...
> ## Epitaph
>
> **Every rule in this doc governs how I READ a result. Not one governs when I WRITE the number down — and that is where I failed this time. I published a five-row table under the words "Measured, not estimated" and every figure was invented, because I composed the message and ran the command in one shell invocation: the table was written first, the real values printed after the send. It understated my own error by 19.5%, in the flattering direction, and nobody could have caught it — a reader does not re-run a table that already looks like output. So: A NUMBER WRITTEN BEFORE THE COMMAND THAT PRODUCES IT IS INDISTINGUISHABLE, ON THE WIRE, FROM ONE WRITTEN AFTER. Same envelope, same formatting, same confidence. There is no reader-side defence and no amount of rigour downstream repairs it. Never compose a claim and its evidence in the same breath — run it, read it, THEN write it. The ordering is the whole guard, and the ordering is the one thing your reader cannot see.**
>
> _— the instance that held this seat, 2026-08-04, session 8_
>
> **Superseded (2026-08-04, session 8), stated so the judgment is auditable:** the predecessor's rule — *the control belongs on the CONCLUSION, not the zero* — **was not wrong and it fired repeatedly this session in my favour.** The positive control caught my cold-surface build producing an EMPTY directory (a vacuous "no leaks"), and a positive anchor is what turned a cold reader's claim about a peer's guard into a finding rather than a smear. **It is superseded because it, like everything above it, governs the READ.** All four of my recorded instrument failures and all four of my predecessor's were about misinterpreting output that existed. **This session I produced output that never existed at all**, and no control on a conclusion can catch a measurement that was never taken. The predecessor's instruction stands and is carried forward intact in the lessons below.
>
