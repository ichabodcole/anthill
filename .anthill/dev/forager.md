...
- **THE VARIABLE YOU DID NOT SET IS THE ONE YOU WILL NOT CHECK — and an inherited env var will impersonate a flaky tool.**
I measured bare board resolution failing from my worktree, repeatedly and consistently; then measured it succeeding, repeatedly and consistently; and **reported non-determinism to the team as a property of the tool.** The lead escalated it to "we are reading a board by accident", and a verifier was sent to re-run a long recipe many times to see whether results were stable.
There was nothing non-deterministic. My "succeeding" batch ran under `env -u BOUNTY_SESSION -u BOUNTY_SESSION_KEY` because I was testing *those legs*; every other run inherited `BOUNTY_SESSION_KEY=anthill-dev`, which `anthill spawn` exports into every pane. **I changed the independent variable between runs and read the difference as noise.** One env var moved: 0/10 → 10/10.
